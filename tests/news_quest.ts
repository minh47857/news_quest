...
describe("news_quest", () => {
  ...
  it("Initialize DAO Config", async () => {
    ...
    assert.equal(daoConfigState.totalQuestions.toNumber(), 0, "Total questions không đúng");
  ...
  it("Create Quest", async () => {
    ...
    const daoConfigState = await program.account.daoConfig.fetch(daoConfig);
    assert.equal(daoConfigState.totalQuestions.toNumber(), 1, "total_questions không tăng");
  });
  ...
  it("End Quest", async () => {
    ...
    try {
      ...
      // Nếu câu hỏi chưa tồn tại, tạo câu hỏi
      await program.methods
        .createQuest(questionId, title, imageUri, choices, new anchor.BN(deadline), rewardPerVote)
        .accounts({
          payer: payer.publicKey,
